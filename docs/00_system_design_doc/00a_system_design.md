...
傳統分床依賴護理師主觀經驗，難以客觀比較。本系統將麻煩度拆分為：

- **客觀因素**：由醫囑系統自動帶入（RASS 分數、用藥複雜度、檢查項目等）
- **主觀因素**：護理師依臨床觀察填寫（照護難度、溝通困難度等）

兩者合計為統一分數，作為分床演算法的主要依據，讓負荷分配有量化基礎。

### 13.2 圖形化拖曳分床

小組長可在視覺化看板上直接拖曳病患至不同護理師欄位，系統即時更新負荷數值與顏色標示，降低調整成本。突發病患轉入時，小組長可立即在看板上指派，不需重新計算整體分配。
...
| 需求 | 系統設計 |
|------|---------|
| **易用性** | 介面以臨床使用習慣設計，拖曳操作直覺；關鍵動作（確認分床、核銷任務）有明確視覺反饋 |
| **即時性** | 麻煩度填寫即時計算總分；建議分床於後端即時運算，前端無需等待重整 |
| **協作性** | 多護理師可同時填寫評估，互不衝突；小組長即時看到最新填寫狀態 |
| **提示機制** | 有未分配病患時封鎖確認；麻煩度未完整填寫時顯示警告 |
| **流程可見性** | 分床狀態（草稿 / 確認）、任務狀態（待辦 / 完成）均在介面清楚呈現 |
