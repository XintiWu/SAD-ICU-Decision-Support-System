...
import { formatSnapshotDateTime, listHandoverSnapshots, subscribeHandoverSnapshots } from '../state/handoverSnapshotStore'

export function AllocationResultPage() {
  const navigate = useNavigate()
  const snapshotItems = useSyncExternalStore(subscribeHandoverSnapshots, listHandoverSnapshots, listHandoverSnapshots)
  const latest = snapshotItems[0] ?? null
        ...
        <div className="min-w-0">
          ...
          {latest ? (
            <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-sm text-slate-600">
              <span>
                <span className="font-semibold text-slate-800">公布時間</span> {formatSnapshotDateTime(latest.createdAt)}
              </span>
              <span className="hidden sm:inline text-slate-300">|</span>
              <span>
                <span className="font-semibold text-slate-800">小組長</span> {latest.createdBy}
              </span>
              <span className="hidden sm:inline text-slate-300">|</span>
              <span>{latest.shiftLabel}</span>
            </div>
          ) : (
            <div className="mt-2 text-sm text-slate-600">
              <span className="font-semibold text-amber-800">尚無交班快照</span> · 請至「指派分床配對」確認送出後再查看
            </div>
          )}
        </div>
        <button
          type="button"
          className="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 ring-1 ring-black/10 hover:bg-black/5"
