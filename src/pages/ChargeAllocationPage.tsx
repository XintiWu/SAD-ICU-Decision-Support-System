...
import { QuickAssignDialog } from '../components/allocation/QuickAssignDialog'
import {
  buildNurseLoadRows,
  computeLoads,
  computeStats,
...
export function ChargeAllocationPage() {
  ...
  const [quickAssignId, setQuickAssignId] = useState<PatientId | null>(null)
  ...
  function movePatient(pid: PatientId, toKey: string) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
          ...
          <div className="relative min-h-0 flex-1 rounded-2xl bg-white ring-1 ring-black/10">
            ...
            >
              <AllocationUnassignedStrip
                ...
                onQuickAssign={setQuickAssignId}
      ...
      {quickAssignId ? (
        <QuickAssignDialog
          patientId={quickAssignId}
          nurseIds={nurseIds}
          loads={loads}
          onSelect={(target) => {
            movePatient(quickAssignId, target === 'unassigned' ? 'unassigned' : target)
            setQuickAssignId(null)
          }}
          onClose={() => setQuickAssignId(null)}
        />
      ) : null}
