...
function ChargeAllocationPageBody() {
  ...
  const chargeNurseId =
    chargeNurseIdFromShift ?? nurses.find((n) => n.role === 'charge_nurse')?.id ?? null
  ...
  async function persistDraft(nextByNurse: Record<string, string[]>, runId: string) {
    ...
    try {
      ...
      mergeStatIntoCatalog(nextCatalog, statOrders)
      setCatalog(new Map(nextCatalog))
  ...
  async function handleRevertToDraft() {
    ...
    try {
      await apiPost(`/allocation-runs/${allocationRunId}/revert-to-draft`, {})
      setRunStatus('draft')
      setError(null)
    } catch (err) {
      setError(err instanceof Error ? err.message : '撤回分床至草稿失敗')
    }
  }
