      ...
      <div className="rounded-2xl bg-[#1a1f2e]/96 px-3 py-2 text-white shadow-[0_20px_50px_-12px_rgba(0,0,0,0.55)] backdrop-blur-md">
        <div className="flex items-center gap-2.5">
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5 text-sm leading-snug">
              <span className="shrink-0 text-xs font-medium text-white/55">拖曳中</span>
              <Sep />
              <span className="shrink-0 text-base font-bold text-white">{patient.bedShort}</span>
              <span className="shrink-0 text-base font-bold text-white">{patient.patientName}</span>
              {patient.badges.map((b) => (
                <span
                  key={b}
                  className={[
                    'shrink-0 rounded px-1 py-px text-[11px] font-bold uppercase',
          ...
          <span
            className={`shrink-0 rounded-full px-2 py-0.5 text-sm font-bold leading-none ${scorePillClass[patient.tone]}`}
          >
            {patient.score}分
          </span>
        </div>
